...
void KDTree<T, dim>::insert(KDTreeNode<T, dim>* node, const Point<T,dim>& point)
{
    ...
    // 3. Compare o valor da coordenada do eixo do ponto p com o valor da coordenada do eixo do nó atual.

    // 4. Se o valor da coordenada do eixo do ponto p for menor que o valor da coordenada do eixo do nó atual,
    // então o ponto p deve ser inserido na subárvore esquerda do nó atual.
    if(point.get(eixo) < node->_point.get(eixo))
    {
        // 4.1. Se a subarvore esquerda for null, crie um novo no com point e aponte para esta subarvore.
        if(node->_left == nullptr)
        {
            KDTreeNode<T, dim>* novoNo = new KDTreeNode<T, dim>(point, (eixo + 1) % dim);
            node->_left = novoNo;
        }
        // 4.2  se não, chame recursivamente esta função para subarvore esquerda.
        else
            this->insert(node->_left, point);
    }
